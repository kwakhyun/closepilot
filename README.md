...
현재 디자인의 [화면 모음](docs/screenshots.md)과 [콘솔 디자인 기준](docs/design-system.md)을 제공합니다. 대시보드의 마감 단계에서 필요한 작업으로 이동하고, 온보딩 설계에서는 브랜드 설정, 월별 작업과 정책 비교를 선택할 수 있습니다.
...
3. 거래별 총액, 환불액, 수수료, 중복과 입금일을 각각 진단합니다. 기존 대표 분류와 승인 식별값은 유지합니다.
4. 감사 기록 화면에서 마감 증빙 JSON을 열어 체크섬, 재계산 결과, 검토 근거와 감사 연결을 검사합니다. 최대 12개 파일을 월별로 비교하며 서버에는 저장하지 않습니다.
5. 온보딩 화면에서 채널별 수수료율을 바꿔 예상 영향과 재검토 대상을 비교합니다. 현재 정책이나 마감 결과에는 적용하지 않습니다.
6. 2020년 1월부터 2035년 12월까지 월을 선택해 같은 브랜드 설정으로 새 합성 작업을 시작합니다. 이전 마감은 먼저 내려받아 보관하며, 이월 검토 건은 증빙과 현재 정산의 주문번호를 대조하는 참고 목록으로 제공합니다.

월별 작업은 계정 기반 보관함이 아닙니다. 세션 전환 후 이전 작업으로 돌아가는 기능, 자동 이월과 회계 처리는 제공하지 않습니다. 임시 메모는 같은 탭에서만 복원할 수 있고 세션 생성 후 6시간이 지나면 사용할 수 없습니다.
...
- Vitest 18개 파일, TypeScript 테스트 186개
- 지정한 domain/application/http/repository/AI 저장소 범위의 커버리지: statements 93.91%, branches 86.93%, functions 94.88%, lines 94.2%
- Playwright 6개: 기존 AI 적용과 취소 시나리오를 합치고, CSV 영향 분석, 메모 복원, 증빙 검사, 정책 비교와 월 전환을 추가했습니다. 320px과 390px 모바일 및 전체 마감 흐름을 포함합니다.
- Kotlin 테스트 11개: `/reconcile`, TypeScript 행별 계약 비교, `/verify`, 변조 거부와 윤년 2월 마감 재계산
