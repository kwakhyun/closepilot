...
- Vitest 21개 파일, TypeScript 테스트 207개
- 지정한 도메인, 애플리케이션, HTTP 처리와 저장소 범위의 커버리지: statements 94.63%, branches 87.79%, functions 97%, lines 95.03%
- Playwright 6개: 기존 AI 적용과 취소 시나리오를 합치고, CSV 영향 분석, 메모 복원, 증빙 검사, 정책 비교와 월 전환을 추가했습니다. 320px과 390px 모바일 및 전체 마감 흐름을 포함합니다.
- Kotlin 테스트 14개: `/reconcile`, TypeScript 행별 계약 비교, `/verify`, 변조 거부, 윤년 2월 마감 재계산, 이전 버전 호환과 정책 변경 패키지 검증
