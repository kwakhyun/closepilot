...
온보딩 프로필은 브랜드명, 산업, 마감 기간, 채널별 요율, 사용 채널, 열 연결, 검토 규칙, 가상 진단 결과와 로드맵을 버전 스냅샷으로 보관한다. CSV 반영 때 `saveMapping=true`이면 검증한 열 연결과 자료 반영이 하나의 명령과 트랜잭션으로 저장된다. 다른 브랜드 프로필을 선택하거나 설정을 복제할 때는 새 세션과 새 합성 데이터를 만들므로 기존 작업공간의 승인 지문과 마감 패키지는 영향을 받지 않는다. 현재 월의 요율 변경은 별도의 `apply_policy` 명령으로 처리한다. 배포 전에 생성된 6시간 세션에는 프로필 필드가 없을 수 있어 기본 LUMIÈRE 프로필로 읽고, 다음 쓰기에서 명시적으로 저장한다.
...
`simulatePolicy`는 복제한 상태에 비교용 요율만 넣어 예상 결과와 승인 무효화 대상을 계산한다. 마감 후에도 비교할 수 있지만 원본 마감은 바뀌지 않는다. `apply_policy`는 미확정 작업의 현재 월 전체에만 적용하며 사유, 근거와 버전을 검사한다. 원본 거래는 유지하고 프로필 버전과 정책 이력을 갱신한다. 재대사 전에는 승인과 마감을 거부한다. 결과 지문이 달라진 승인에는 `invalidatedByPolicy`를 남겨 요율을 되돌려도 재승인을 요구하고 기존 메모는 보존한다.

`record_followup`은 같은 보관함과 프로필의 이전 확정 마감을 서버에서 조회한다. 원본 해시와 이월 승인 지문을 검증하고 현재 `(channel, orderId)` 정산 근거를 연결한다. 근거 검토 완료에는 재대사와 정산 선택이 필요하다. 후보 자료의 지문이 달라지면 이전 기록은 재검토 대상으로 표시한다. 금액, 원본 승인이나 이전 마감은 바꾸지 않으며 명령, 감사 기록과 처리 영수증은 함께 커밋한다.

규칙 버전 `krw-net-v1.2.0`은 정책 변경과 이월 검토 기록을 추가한다. 금액 공식과 거래 분류는 그대로다. 기존 `krw-net-v1.1.0` 패키지도 TypeScript와 Kotlin에서 검증하며 원본 해시와 내보내기 버전을 보존한다. 새 패키지는 선택적으로 정책 이력과 이월 기록을 포함한다. 패키지 검사만으로 외부 원본 마감의 소유권이나 은행 입금을 증명할 수는 없다.
...
새 월은 `period`(2020-01~2035-12)로 선택한다. 현재 설정을 같은 브랜드로 복제하는 경우 프로필 ID를 유지하되 기간과 월말 기준일을 바꾸고 새 합성 거래를 만든다. 이전 작업의 자료와 마감은 수정하지 않는다. 증빙의 이월 검토 목록은 같은 프로필의 더 늦은 월에서 `(channel, orderId)`가 일치하는 정산 행 수를 보여 주는 참고 기능이다. 자동 이월이나 입금 확정은 수행하지 않는다.

마이그레이션 4는 브라우저별 보관함 `closepilot_library`와 6시간 접근 권한 `closepilot_access_sessions`를 분리한다. 새 작업은 생성일로부터 30일간 최대 12개를 보관하며, 브라우저의 별도 HttpOnly bearer 쿠키로 목록과 복귀를 허용한다. 목록의 UUID는 선택자일 뿐 접근 권한이 아니며 서버가 소유 해시를 함께 검사한다. 쿠키 삭제 시 계정 기반 복구는 제공하지 않는다. 세션이 없으면 마지막으로 열었던 작업을 재개하며, 실패한 명령은 자동 재실행하지 않는다.

복귀는 보관함의 마지막 열람 시각과 접근 권한만 갱신한다. 마감 상태, JSONB 내용, 승인, 감사와 보관 만료일은 바꾸지 않는다. 기존 배포의 6시간 작업은 자동 편입하거나 연장하지 않는다. 최초 접근 권한의 만료 기록은 남겨 기존 토큰 조회로 우회하지 못하게 한다. 새 접근 권한은 이전 복귀 권한을 교체한다. 클라이언트 명령은 `X-Workspace-Scope`를 보내며 저장소는 행 잠금 안에서 검사해, 다른 탭에서 전환한 동일 버전 작업에 오래된 명령이 적용되는 것을 거부한다.

CSV 열 연결은 같은 보관함과 같은 프로필의 저장된 매핑 스냅샷에서 선택한다. 별도의 거래나 정책 변경 없이 현재 파일에 적용해 다시 검증한다. 필수 열 이름이 없으면 적용을 차단하고 누락된 열을 표시한다. 선택적인 입금일 열만 없으면 연결을 비운다. 새 이름을 붙인 독립 템플릿 편집기는 아니며 보관 기간과 개수 제한을 공유한다.

## 탭별 임시 메모

조회 DTO의 `draftScope`와 `reviewFingerprints`로 작업별 임시 저장 키와 메모의 결과 버전을 구분한다. `draftScope`는 서버가 만든 비인증용 식별값이며 세션 토큰이 아니다. 메모는 `sessionStorage`에 저장하고 새 저장 시점부터 최대 6시간, 작업 생성 후 최대 30일의 만료 시각을 검사한다. 기존 메모의 만료 시각은 소급 변경하지 않는다. 서버 승인이나 패키지에 포함하지 않으며 복원 시 확인란을 해제한다. 승인 성공 후 해당 메모를 제거한다.
...
stateDiagram-v2
  [*] --> review: 고정 샘플 / 초기 대사
  review --> open: CSV 반영 / 정책 변경
  open --> open: CSV 추가 반영 / 정책 변경
  open --> review: 재대사
...
| `closepilot_library`           | 브라우저 보관함 소유 해시, 작업 선택자와 마지막 열람 시각      |
| `closepilot_access_sessions`   | 보관된 작업에 접근하는 6시간 토큰의 해시와 만료 시각           |
...
보관함 복귀 후에도 AI 한도와 캐시는 접근 토큰이 아니라 기존 작업 키에 연결된다. 세션을 다시 열어도 작업별 사용량 한도를 초기화하지 않는다. 새 보관함 테이블은 [마이그레이션 4](../migrations/004_workspace_library.sql)에 정의한다.
