...
AI 검토 메모 초안 기능을 사용하려면 서버 전용 `OPENAI_API_KEY`를 설정한다. 기본 모델은 `gpt-5.6-luna`이며 `OPENAI_REVIEW_MODEL`로 바꿀 수 있다. 두 변수 모두 브라우저 번들에 포함하지 않는다. 키가 없거나 호출 또는 출력 검증에 실패하면 규칙 기반 초안으로 전환되므로 대사, 검토, 마감 흐름은 계속 사용할 수 있다.

## 배포

AI 호출 제한과 캐시에는 마이그레이션 3의 `closepilot_review_drafts` 테이블이 필요하다. 런타임은 기존 마이그레이션 잠금 안에서 이를 추가한다. 세션별 시간당 10회, 전체 일당 100회 생성 시도를 UTC 기준으로 제한하고 동시 실행은 세션당 1건, 전체 4건으로 제한한다. 제한 시에는 HTTP 200의 규칙 기반 초안과 사유가 표시된다. 이미 시작한 생성의 실패도 횟수에 포함되므로 반복 재시도로 한도를 소모하지 않도록 한다.

성공한 초안은 같은 세션의 동일 근거, 모델과 프롬프트 버전에서 재사용한다. 근거나 프롬프트를 바꿨으면 새 캐시 키로 생성하며, 캐시 조회에는 `X-Review-Draft-Cache: hit`가 붙는다. 프롬프트나 출력 검증 정책을 변경할 때는 `REVIEW_DRAFT_PROMPT_VERSION`도 갱신한다. 프로세스가 종료돼 실행 예약이 남아도 45초 뒤 만료된다. 설정 복제와 AI 캐시 테이블은 기존 승인 및 마감 파일을 소급 변경하지 않는다.
...
| AI 초안이 규칙 기반으로 전환됨   | `review_draft_fallback` 로그의 requestId, errorType과 errorCode를 확인한다. AI_RATE_LIMITED 또는 AI_BUSY이면 호출량과 실행 중인 요청을 확인한다. |
...
모든 API 응답의 `Server-Timing`과 구조화 로그로 오류율, p95, 명령 수행 시간, AI 전환율을 집계한다. 지표 정의와 초기 경보 기준은 [운영 지표와 경보 기준](observability.md)을 따른다.
...
배포 후 장애가 발생하면 Vercel에서 이전 정상 배포로 되돌린다. DB도 변경했다면 이전 앱과의 호환성을 먼저 확인한다. 현재 마이그레이션은 초기 스키마 생성, JSONB 객체 제약, AI 초안 저장소 추가의 세 개이며, 스키마를 자동으로 되돌리는 기능은 없다. 스키마 변경은 이전 앱과도 호환되는 확장부터 적용한다.
...
`Dockerfile`은 Next.js standalone 빌드 결과를 `root`가 아닌 사용자로 실행하도록 설정되어 있다. `compose.yaml`은 로컬 PostgreSQL과 웹 서비스를 함께 실행하며 DB 포트를 외부에 공개하지 않도록 구성했다. 포함된 DB 비밀번호는 로컬 예제이므로 운영 서버에 사용하면 안 된다. 두 파일은 코드로 확인했으며 웹 앱의 컨테이너 빌드와 Compose 실행은 검증하지 않았다. 저장소 통합 테스트에 사용한 임시 PostgreSQL 컨테이너와는 별도의 검증 범위다.
...
`bash gradlew run --args='--server 8081'`은 `127.0.0.1`에만 바인딩되는 Kotlin 대사 및 검증 REST 경계를 연다. `POST /reconcile`은 `krw-net-v1.1.0` 규칙 버전, 채널별 `feeBps`, 주문과 정산 입력을 받아 행별 분류와 합계를 반환한다. `POST /verify`는 최대 5MB의 JSON 마감 패키지를 받고 CLI와 같은 독립 재계산 함수를 호출한다. 요청과 응답 형식은 [Kotlin OpenAPI 계약](kotlin-openapi.yaml)에 고정했다.

이 서버는 로컬과 CI의 계약 검증용이며 인증, TLS, 외부 공개 운영을 제공하지 않는다. 공개 Vercel 데모는 Kotlin 서비스를 호출하지 않고 TypeScript 인프로세스 엔진을 사용한다. Kotlin 런타임을 별도 컨테이너로 배포하려면 인증, TLS, 타임아웃, 재시도, 관측, 네트워크 정책을 먼저 추가해야 한다.
