...
- 마감 증빙 조회는 최대 5MB JSON을 받는다. 파일을 서버에 보관하지 않으므로 원본 다운로드를 따로 보관한다. 검사 실패를 피하려고 해시나 검토 내용을 직접 수정하지 않는다.
- 새 월로 전환하기 전에 현재 마감의 CSV와 JSON을 내려받는다. 전환하면 쿠키가 새 세션을 가리키며 이전 세션으로 돌아가는 UI는 없다. 이월 검토 목록의 연결 행 수는 은행 입금 확인이 아니다.
- 수수료 비교는 읽기 전용이다. 비교값을 현재 정책으로 저장하는 기능은 제공하지 않는다.
